...
def train_models(sitedata, grid_data, output_dir="models"):
    ...
    try:
        ...
        for fold, (train_idx, test_idx) in enumerate(kf.split(sitedata)):
            train_fold = sitedata.iloc[train_idx].replace([np.inf, -np.inf], np.nan).dropna(subset=["Easting", "Northing", "time", "NDVI", "log_NO2"])
            test_fold = sitedata.iloc[test_idx].replace([np.inf, -np.inf], np.nan).dropna(subset=["Easting", "Northing", "time", "NDVI", "NO2"])
            
            if train_fold.empty or test_fold.empty:
                print(f"⚠️ RF Fold {fold + 1} skipped (empty data after cleaning).")
                continue
        ...
        for fold, (train_idx, test_idx) in enumerate(kf.split(sitedata)):
            ...
            if train_fold.empty or test_fold.empty:
                print(f"⚠️ GP Fold {fold + 1} skipped (empty data after cleaning).")
                continue
